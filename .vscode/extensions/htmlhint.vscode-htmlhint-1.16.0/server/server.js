...
const htmlhint = __importStar(require("htmlhint"));
...
let workspaceRootCache = new Map();
let settings = null;
let linter = null;
/**
 ...
 */
function makeDiagnostic(problem, _document) {
    ...
    if (settings?.htmlhint) {
        ...
        if (settings.htmlhint.configFile) {
            if (fs.existsSync(settings.htmlhint.configFile)) {
                ...
                trace(`[HTMLHint Debug] Using configFile setting: ${settings.htmlhint.configFile}`);
            }
            else {
            ...
            trace(`[HTMLHint Debug] Using options setting: ${JSON.stringify(settings.htmlhint.options)}`);
        }
            ...
            if (fs.existsSync(settings.htmlhint.optionsFile)) {
                ...
                trace(`[HTMLHint Debug] Using optionsFile setting: ${settings.htmlhint.optionsFile}`);
            }
            else {
                trace(`[HTMLHint Debug] optionsFile does not exist: ${settings.htmlhint.optionsFile}, falling back to file search`);
                options = findConfigForHtmlFile(filePath);
            }
        }
        else {
            trace(`[HTMLHint Debug] No explicit config specified, searching for .htmlhintrc`);
            options = findConfigForHtmlFile(filePath);
        }
    }
    else {
...
function loadConfigurationFile(configFile) {
    ...
    if (fs.existsSync(configFile)) {
        ...
        try {
            ...
            trace(`[HTMLHint Debug] Successfully parsed config: ${JSON.stringify(ruleset)}`);
        }
        catch (e) {
...
function isHtmlHintRuleEnabled(value) {
...
function isRuleEnabledForDocument(document, ruleId) {
    try {
        const parsedUri = vscode_uri_1.URI.parse(document.uri);
        if (parsedUri.scheme !== "file") {
            trace(`[DEBUG] isRuleEnabledForDocument: Non-file scheme for ${document.uri}, rule ${ruleId} treated as disabled`);
            return false;
        }
        const config = getConfiguration(parsedUri.fsPath);
        const ruleValue = config[ruleId];
        const enabled = isHtmlHintRuleEnabled(ruleValue);
        trace(`[DEBUG] isRuleEnabledForDocument: Rule ${ruleId} enabled=${enabled} for ${document.uri}`);
        return enabled;
    }
    catch (error) {
        trace(`[DEBUG] isRuleEnabledForDocument: Failed to determine rule ${ruleId} for ${document.uri}: ${error}`);
        return false;
    }
}
...
function validateAllTextDocuments(connection, documents) {
    ...
    if (!settings) {
        trace(`[DEBUG] Settings not loaded yet, skipping validation of all documents`);
        return;
    }
...
let connection = (0, node_1.createConnection)();
...
function createHtmlLangRequireFix(document, diagnostic) {
    trace(`[DEBUG] createHtmlLangRequireFix called with diagnostic: ${JSON.stringify(diagnostic)}`);
    if (!diagnostic.data || diagnostic.data.ruleId !== "html-lang-require") {
        trace(`[DEBUG] createHtmlLangRequireFix: Invalid diagnostic data or ruleId`);
        return null;
    }
    ...
    trace(`[DEBUG] createHtmlLangRequireFix: Found html tag at index ${htmlTagStart}: ${htmlTag}`);
    ...
    trace(`[DEBUG] createHtmlLangRequireFix: Will insert "${newText}" at position ${JSON.stringify(insertPosition)}`);
    const edit = {
        range: {
...
function createMetaCharsetRequireFix(document, diagnostic) {
    ...
    const headContent = headMatch[2];
    const charsetMatch = headContent.match(/<meta\s+charset\s*=\s*["'][^"']*["'][^>]*>/i);
    if (charsetMatch) {
...
function createMetaViewportRequireFix(document, diagnostic) {
    ...
    const headContent = headMatch[2];
    const viewportMatch = headContent.match(/<meta\s+name\s*=\s*["']viewport["'][^>]*>/i);
    if (viewportMatch) {
    ...
    const metaCharsetMatch = headContent.match(/<meta\s+charset\s*=\s*["'][^"']*["'][^>]*>/i);
    const shouldSelfClose = isRuleEnabledForDocument(document, "tag-self-close");
    trace(`[DEBUG] createMetaViewportRequireFix: tag-self-close enabled=${shouldSelfClose}`);
...
function createMetaDescriptionRequireFix(document, diagnostic) {
    ...
    const headContent = headMatch[2];
    const descriptionMatch = headContent.match(/<meta\s+name\s*=\s*["']description["'][^>]*>/i);
    if (descriptionMatch) {
    ...
    const headStart = headMatch.index + headMatch[0].indexOf(">") + 1;
    const metaCharsetMatch = headContent.match(/<meta\s+charset\s*=\s*["'][^"']*["'][^>]*>/i);
    const metaViewportMatch = headContent.match(/<meta\s+name\s*=\s*["']viewport["'][^>]*>/i);
    let insertPosition;
    const shouldSelfClose = isRuleEnabledForDocument(document, "tag-self-close");
 ...
 */
function createLinkRelCanonicalRequireFix(document, diagnostic) {
    if (!diagnostic.data ||
    ...
    const headContent = headMatch[2];
    const canonicalMatch = headContent.match(/<link\s+[^>]*rel\s*=\s*["']canonical["'][^>]*>/i);
    if (canonicalMatch) {
    ...
    const metaCharsetMatch = headContent.match(/<meta\s+charset\s*=\s*["'][^"']*["'][^>]*>/i);
    const metaViewportMatch = headContent.match(/<meta\s+name\s*=\s*["']viewport["'][^>]*>/i);
    const metaDescriptionMatch = headContent.match(/<meta\s+name\s*=\s*["']description["'][^>]*>/i);
    let insertPosition;
    const shouldSelfClose = isRuleEnabledForDocument(document, "tag-self-close");
    const canonicalSnippet = '\n    <link rel="canonical" href=""' + (shouldSelfClose ? " />" : ">");
    if (metaDescriptionMatch) {
        // Insert after description meta tag
        const metaDescriptionEnd = headStart + metaDescriptionMatch.index + metaDescriptionMatch[0].length;
...
function createAltRequireFix(document, diagnostic) {
    ...
    const line = lines[diagnostic.data.line - 1];
    if (!line) {
        trace(`[DEBUG] createAltRequireFix: No line found at ${diagnostic.data.line}`);
        return null;
    }
...
function createButtonTypeRequireFix(document, diagnostic) {
    if (!diagnostic.data || diagnostic.data.ruleId !== "button-type-require") {
        trace(`[DEBUG] createButtonTypeRequireFix: Invalid diagnostic data or ruleId`);
        return null;
    }
    const text = document.getText();
    const lines = text.split("\n");
    const line = lines[diagnostic.data.line - 1];
    if (!line) {
        trace(`[DEBUG] createButtonTypeRequireFix: No line found at ${diagnostic.data.line}`);
        return null;
    }
...
function createAttrNoUnnecessaryWhitespaceFix(document, diagnostic) {
    if (!diagnostic.data ||
        ...
        trace(`[DEBUG] createAttrNoUnnecessaryWhitespaceFix: Invalid diagnostic data or ruleId`);
        return null;
    }
    const text = document.getText();
    const lines = text.split("\n");
    const line = lines[diagnostic.data.line - 1];
    if (!line) {
        trace(`[DEBUG] createAttrNoUnnecessaryWhitespaceFix: No line found at ${diagnostic.data.line}`);
        return null;
    }
 ...
 */
function createAttrWhitespaceFix(document, diagnostic) {
    trace(`[DEBUG] createAttrWhitespaceFix called with diagnostic: ${JSON.stringify(diagnostic)}`);
    if (!diagnostic.data ||
    ...
    const line = lines[diagnostic.data.line - 1];
    if (!line) {
        trace(`[DEBUG] createAttrWhitespaceFix: No line found at ${diagnostic.data.line}`);
        return null;
    }
 ...
 */
function createTagSelfCloseFix(document, diagnostic) {
    trace(`[DEBUG] createTagSelfCloseFix called with diagnostic: ${JSON.stringify(diagnostic)}`);
    if (!diagnostic.data || diagnostic.data.ruleId !== "tag-self-close") {
        trace(`[DEBUG] createTagSelfCloseFix: Invalid diagnostic data or ruleId: ${JSON.stringify(diagnostic.data)}`);
        return null;
    }
    trace(`[DEBUG] createTagSelfCloseFix: Valid diagnostic for tag-self-close rule`);
    ...
    if (raw.endsWith(">")) {
        ...
        const lineText = text.substring(tagStart, text.indexOf("\n", tagStart) !== -1
            ? text.indexOf("\n", tagStart)
            : text.length);
        ...
        if (closeTagIndex !== -1) {
            ...
            trace(`[DEBUG] createTagSelfCloseFix: Could not find closing '>' for tag`);
            return null;
        }
    }
    if (text[tagEndIndex] !== ">") {
        trace(`[DEBUG] createTagSelfCloseFix: Unexpected tag ending: ${text[tagEndIndex]} at position ${tagEndIndex}`);
        trace(`[DEBUG] Text around position: ${text.substring(tagEndIndex - 10, tagEndIndex + 10)}`);
        return null;
    }
    trace(`[DEBUG] createTagSelfCloseFix: Found tag ending '>' at position ${tagEndIndex}`);
    ...
    trace(`[DEBUG] createTagSelfCloseFix: Created edit to replace '>' with ' />'`);
    const action = node_1.CodeAction.create("Add self-closing tag", {
        changes: {
            [document.uri]: [edit],
        },
    ...
    trace(`[DEBUG] createTagSelfCloseFix: Created code action: ${JSON.stringify(action)}`);
    return action;
}
/**
 * Create auto-fix action for spec-char-escape rule
 */
function createSpecCharEscapeFix(document, diagnostic) {
    if (!diagnostic.data ||
 ...
 */
function createTagNoObsoleteFix(document, diagnostic) {
    trace(`[DEBUG] createTagNoObsoleteFix called with diagnostic: ${JSON.stringify(diagnostic)}`);
    ...
    for (const tag of acronymTags) {
        const distance = Math.abs(tag.line - diagnosticLine) +
 ...
 */
function findOpeningTag(tags, closingTag) {
 ...
 */
function findClosingTag(tags, openingTag) {
 ...
 */
function findTagBoundaries(text, position) {
 ...
 */
function createAttrNoDuplicationFix(document, diagnostic) {
    trace(`[DEBUG] createAttrNoDuplicationFix called with diagnostic: ${JSON.stringify(diagnostic)}`);
 ...
 */
function createAttrValueNoDuplicationFix(document, diagnostic) {
    trace(`[DEBUG] createAttrValueNoDuplicationFix called with diagnostic: ${JSON.stringify(diagnostic)}`);
    if (!diagnostic.data ||
        diagnostic.data.ruleId !== "attr-value-no-duplication") {
        trace(`[DEBUG] createAttrValueNoDuplicationFix: Invalid diagnostic data or ruleId`);
        return null;
    }
    const text = document.getText();
    const diagnosticOffset = document.offsetAt(diagnostic.range.start);
    ...
    while ((match = attrPattern.exec(tagContent)) !== null) {
        ...
        if (attrValue.trim()) {
            ...
            if (values.length !== uniqueValues.length) {
                ...
                trace(`[DEBUG] createAttrValueNoDuplicationFix: Will replace "${fullMatch}" with "${newFullMatch}"`);
 ...
 */
function createFormMethodRequireFix(document, diagnostic) {
    trace(`[DEBUG] createFormMethodRequireFix called with diagnostic: ${JSON.stringify(diagnostic)}`);
    if (!diagnostic.data || diagnostic.data.ruleId !== "form-method-require") {
        trace(`[DEBUG] createFormMethodRequireFix: Invalid diagnostic data or ruleId`);
        return null;
    }
    const text = document.getText();
    const diagnosticOffset = document.offsetAt(diagnostic.range.start);
    ...
    if (methodAttrMatch) {
        trace(`[DEBUG] createFormMethodRequireFix: Method attribute already exists`);
        return null;
    }
    ...
    if (!formMatch) {
        trace(`[DEBUG] createFormMethodRequireFix: Could not parse form tag structure`);
        return null;
    }
    ...
    trace(`[DEBUG] createFormMethodRequireFix: Will insert "${newText}" at position ${insertPosition}`);
    const workspaceEdit = {
        changes: {
            [document.uri]: [edit],
        },
 ...
 * - <hr> → <hr/>
 */
function createEmptyTagNotSelfClosedFix(document, diagnostic) {
    trace(`[DEBUG] createEmptyTagNotSelfClosedFix called with diagnostic: ${JSON.stringify(diagnostic)}`);
    if (!diagnostic.data ||
        diagnostic.data.ruleId !== "empty-tag-not-self-closed") {
        trace(`[DEBUG] createEmptyTagNotSelfClosedFix: Invalid diagnostic data or ruleId: ${JSON.stringify(diagnostic.data)}`);
        return null;
    }
    const text = document.getText();
    const diagnosticOffset = document.offsetAt(diagnostic.range.start);
    ...
    const tagBoundaries = findTagBoundaries(text, diagnosticOffset);
    if (!tagBoundaries) {
        trace(`[DEBUG] createEmptyTagNotSelfClosedFix: Could not find tag boundaries`);
        return null;
    }
    const { tagStart, tagEnd } = tagBoundaries;
    ...
    while ((match = voidElementPattern.exec(searchText)) !== null) {
        ...
        if (matchStart <= diagnosticOffset && diagnosticOffset <= matchEnd) {
            ...
            if (!voidElements.includes(tagName)) {
                trace(`[DEBUG] createEmptyTagNotSelfClosedFix: ${tagName} is not a void element`);
                return null;
            }
            trace(`[DEBUG] createEmptyTagNotSelfClosedFix: Found non-self-closing ${tagName} tag at position ${matchStart}-${matchEnd}`);
            ...
            trace(`[DEBUG] createEmptyTagNotSelfClosedFix: Will replace "${fullMatch}" with "${selfClosingTag}"`);
            const action = node_1.CodeAction.create(`Convert ${tagName} tag to self-closing`, {
                changes: {
                    [document.uri]: [edit],
                },
            }, node_1.CodeActionKind.QuickFix);
    ...
    trace(`[DEBUG] createEmptyTagNotSelfClosedFix: No matching empty tag pair found`);
    return null;
}
/**
...
connection.onInitialize((_params, _token) => {
    ...
    try {
        ...
        if (settings.htmlhint.ignoreGitignore) {
            ...
            if (workspaceRoot && shouldIgnoreFile(fsPath, workspaceRoot)) {
                trace(`[DEBUG] File ${fsPath} is ignored by .gitignore, skipping validation`);
...
connection.onRequest("textDocument/codeAction", async (params) => {
    ...
    try {
        ...
        trace(`[DEBUG] Context diagnostics: ${JSON.stringify(context.diagnostics)}`);
        ...
        const filteredDiagnostics = context.diagnostics.filter((diagnostic) => {
            ...
            if (!diagnostic.range ||
                ...
                trace(`[DEBUG] Skipping diagnostic with invalid range: ${JSON.stringify(diagnostic)}`);
                return false;
            }
            const diagnosticRange = diagnostic.range;
            return (diagnosticRange.start.line <= normalizedRange.end.line &&
                diagnosticRange.end.line >= normalizedRange.start.line);
        });
        trace(`[DEBUG] Filtered diagnostics: ${JSON.stringify(filteredDiagnostics)}`);
        ...
        for (const diagnostic of filteredDiagnostics) {
            trace(`[DEBUG] Creating fixes for diagnostic: ${JSON.stringify(diagnostic)}`);
            trace(`[DEBUG] Diagnostic data: ${JSON.stringify(diagnostic.data)}`);
            ...
            const enhancedDiagnostic = {
                ...
                data: diagnostic.data || {
                    ...
                    raw: diagnostic.code === "tag-self-close"
                        ? document
                            .getText()
                            .substring(document.offsetAt(diagnostic.range.start), document.offsetAt(diagnostic.range.end))
                        : diagnostic.message.split(" ")[0],
                },
            };
            const fixes = await createAutoFixes(document, [enhancedDiagnostic]);
            trace(`[DEBUG] Created ${fixes.length} fixes for diagnostic: ${JSON.stringify(enhancedDiagnostic)}`);
            codeActions.push(...fixes.map((fix) => ({
                title: fix.title,
                kind: fix.kind,
                ...
                edit: fix.edit
                    ? {
                        changes: fix.edit.documentChanges
                            ? {
                                [uri]: fix.edit.documentChanges[0].edits || [],
                            }
 ...
 */
function shouldIgnoreFile(filePath, workspaceRoot) {
    try {
        ...
        const ig = (0, ignore_1.default)().add(gitignoreContent);
