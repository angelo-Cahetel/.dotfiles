...
(function (TrackingReasonMask) {
     ...
     */
    TrackingReasonMask[TrackingReasonMask["ForceImported"] = 8] = "ForceImported";
...
class TrackingMap {
    ...
    *walkInActiveAndExpiredURIs(beforeTimestamp) {
        for (let [uri, item] of this.trackingMap) {
    ...
    checkImportedRecursively(uri, depth = 5) {
        let item = this.trackingMap.get(uri);
        if (!item) {
            return;
        }
    ...
    isImportedAncestrally(uri, depth = 5) {
        let item = this.trackingMap.get(uri);
        if (!item) {
            return false;
        }
