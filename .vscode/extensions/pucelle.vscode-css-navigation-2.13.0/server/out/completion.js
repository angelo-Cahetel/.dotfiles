...
async function getCompletionItemsInHTML(fromPart, currentService, document, cssServiceMap, configuration, offset) {
    ...
    if (fromPart.isHTMLType()) {
        labels.add(languages_1.CompletionLabelType.Definition, currentService.getCompletionLabels(matchPart, fromPart, configuration.maxHoverStylePropertyCount));
        labels.add(languages_1.CompletionLabelType.Definition, await cssServiceMap.getCompletionLabels(matchPart, fromPart, configuration.maxHoverStylePropertyCount));
    }
    ...
    // It's a little different with css document, don't want it visits all html files.
        ...
        if (fromPart.isCSSVariableType()) {
            labels.add(languages_1.CompletionLabelType.CSSVariable, await cssServiceMap.getCompletionLabels(matchPart, fromPart, configuration.maxHoverStylePropertyCount));
        }
    }
